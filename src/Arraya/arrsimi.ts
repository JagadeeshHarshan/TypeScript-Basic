...
let movies: Movie[] = [ { title: "Inception", genres: ["Sci-Fi", "Action", "Thriller"] },
{ title: "Intersteller", genres: ["Sci-Fi", "Drama"] },
{ title: "Avengers", genres: ["Action", "Superhero"] },
{ title: "Titanic", genres: ["Romance", "Drama"]}
];

let target = movies.find(m => m.title === "Inception");
let recommendations = movies .filter(m => m.title !== target?.title)
.map(m => {
